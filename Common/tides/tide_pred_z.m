...
% USAGE:
% [TS,ConList,Deph]=tide_pred_z(Model,SDtime,lat,lon,type,Cid);
% 
% PARAMETERS
% Input:
...
%                               constituents (char*4)
%            Deph(N) or Deph(N,M) - interpolated depths
%
...
%
function [TS,conList,Deph]=tide_pred_z(Model,SDtime,lat,lon,type,Cid);
...
[amp,pha,Deph,conList]=extract_HC(Model,lat,lon,type);
cph= -i*pha*pi/180;
...
if nargin<=5,
 ...
 elseif DriftTrack==1,
  ...
  if isempty(Cid)==0,
   ...
   if TimeSeries==1,
    TS=harp1(time,hc(Cid,:),conList(Cid,:));
   elseif DriftTrack==1,
    for k=1:N
     TS(k)=harp1(time(k),hc(Cid,k),conList(Cid,:));
    end
   else % Map
    for k=1:nc
     hci(:,:,k)=hc(k,:,:);
    end
    TS=harp(time,hci(:,:,Cid),conList(Cid,:));   % Previous bug: fixed
                                                 % 5/20/2005
   end
